...
class MyProfile2: UIViewController, UITableViewDelegate, UITableViewDataSource, MyTableViewCellDelegate2 {
    ...
    let applyButton : UIButton = {
        ...
        button.setTitle("Post Service", for: .normal)
        button.setTitleColor(UIColor.white, for: .normal)
        button.addTarget(self, action: #selector(postServicePressed), for: .touchUpInside)
...
//        bottomBar3.heightAnchor.constraint(equalToConstant: 2).isActive = true
        ...
        Database.database().reference().child("Users").child(Auth.auth().currentUser!.uid).child("name").observe(.value) { (snapshot) in
            if let userName : String = (snapshot.value as? String) {
                ...
                self.profileName.text = "Your Profile - Unknown Name"
            }
        }
        ...
        if self.findingRating == true {
            ...
            Database.database().reference().child("Users").child(Auth.auth().currentUser!.uid).child("number-of-ratings").observeSingleEvent(of: .value) { (ratingNum) in
                if let value = ratingNum.value as? NSNumber {
                    let newNumber = Float(truncating: value)
                    if newNumber == 0 {
...
//        self.navigationController?.navigationBar.barTintColor = UIColor.white
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        self.myServices.text = "My Skills"
    ...
    func addEtraView() {
        ...
        self.applyButton.topAnchor.constraint(equalTo: self.noServiceLabel.bottomAnchor, constant: 20).isActive = true
